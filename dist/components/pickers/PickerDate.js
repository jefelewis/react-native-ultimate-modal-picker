...
const PickerDate = (props) => {
    ...
    const renderPicker = () => {
        ...
        if (react_native_1.Platform.OS === 'android' && androidModalVisible === true) {
            return (jsx_runtime_1.jsx(datetimepicker_1.default, { mode: "date", display: props.mode, value: date, onChange: (event, date) => selectDate(event, date) }, void 0));
        }
        // Platform: iOS
        else if (react_native_1.Platform.OS === 'ios') {
            // Major Version iOS
            ...
            if (majorVersionIOS >= 14) {
                return (jsx_runtime_1.jsx(datetimepicker_1.default, { mode: "date", value: tempDate ? tempDate : date, onChange: (event, newDate) => selectDate(event, newDate) }, void 0));
            }
            // iOS 13 And Under
            else {
                // <Modal
