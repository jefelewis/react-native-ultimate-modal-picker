...
const PickerTime = (props) => {
    ...
    const renderPicker = () => {
        ...
        if (react_native_1.Platform.OS === 'android' && androidModalVisible === true) {
            return (<datetimepicker_1.default mode="time" display={props.mode} value={date} onChange={(event, date) => selectDate(event, date)}/>);
        }
        // Platform: iOS
        else if (react_native_1.Platform.OS === 'ios') {
            // Major Version iOS
            ...
            if (majorVersionIOS >= 14) {
                return (<datetimepicker_1.default mode="datetime" value={tempDate ? tempDate : date} onChange={(event, newDate) => selectDate(event, newDate)}/>);
            }
            // iOS 13 And Under
            else {
                // <Modal
